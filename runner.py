...
class CacheManager:
    ...
    def generate_fingerprint(self, html_content: str) -> tuple[str, str]:
        # 💡 ИНТЕЛЛЕКТУАЛЬНЫЙ FINGERPRINT (Возвращает хэш и саму строку для анти-флап защиты)
        soup = BeautifulSoup(html_content, 'lxml')
    ...
    def check_cache(self, url: str, html_content: str) -> tuple[bool, Optional[dict]]:
        ...
        # 💡 ANTI-FLAKE PROTECTION (ЗАЩИТА ОТ ДИНАМИЧЕСКИХ СБОЕВ)
        # Если страница прогрузилась с "0" ценой или без H1, не инвалидируем хороший кэш!
